...
* `BBLowestShippingCostDict`: most keys may be missing
...
* Add more type hints
* Explicitly expose `__author__` and `__version__`
...
* Add `TypedDict`s for the return type hint of select endpoints: `BBProductCategoryDict`, `BBProductInformationDict`
...
* Deprecate `get_products_stock`, `get_product_stock`, `get_products_variations_stock`
  and `get_product_variations_stock`, because the endpoint they’re calling are deprecated and will be unsupported from
  April 1st, 2024
